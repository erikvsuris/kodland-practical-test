...
def update(delta_time):
    ...
    elif not music_on and music.is_playing("background"):
        music.stop()
...
def draw_menu():
    ...
    music_text = f"Music and Sound: {"On" if music_on else "Off"}"
...
def draw_game():
    ...
    for i in range(PLAYER.life):
        screen.blit('heart', ((i * TILE_SIZE) + TILE_SIZE, TILE_SIZE))


def draw_over():
    screen.fill("black")
    screen.draw.text("Game Over", center=(WIDTH/2, HEIGHT/2 - 20), color="white", fontsize=40)
    screen.draw.text("Press ENTER to return menu", center=(WIDTH/2, HEIGHT/2 + 20), color="white", fontsize=30)
